...
const Button = forwardRef<React.FunctionComponent<ButtonProps>, ButtonProps>(
  (props, ref) => {
    ...
    switch (props.variant) {
      ...
      case "outline":
        ButtonComponent = (
          <button onClick={props.onClick} css={styleOutline(props)}>
            {props.children}
          </button>
        );
        break;
      case "dropdown":
        ButtonComponent = (
          <button {...props} onClick={props.onClick} css={styleDropdown(props)}>
            {props.children}
            <div>&#8744;</div>
          </button>
        );
...
const styleSolid = ({ size = "md", color = "blue2" }: ButtonProps) => {
...
const styleOutline = ({ size = "md", color = "blue2" }: ButtonProps) => {
  const height: string = handlingButtonDetail.height[size];
  const fontSise: string = handlingButtonDetail.fontSise[size];
  const background = Colors[color];
  return css`
